from locust import task, FastHttpUser, SequentialTaskSet, between,constant
...
class UserBehaviour(SequentialTaskSet):
    def __init__(self, parent):
        ...
        self.basic_auth_header = self._get_basic_auth_header("NCTSTraderUser", "3vAT.2G98Ar!")
        self.guildID=""
    def _get_basic_auth_header(self, username, password):
        # Helper method to generate the Basic Auth header
    ...
    def on_start(self):
        ...
        with self.client.post(
                ...
                headers=self.basic_auth_header,  # Basic Auth credentials
                name="login",
                catch_response=True
        ) as res:
    ...
    #                 res.success()
    #
    #                 # Debugging: print the entire response data to understand its structure
    #                 print(f"Response Data: {json.dumps(response_data, indent=4)}")  # Pretty print the full response
    #
    #                 # Access the messages list from the data
    #                 messages = response_data.get("data", {}).get("messages", [])  # Access messages list
    #                 msg_indexes = []  # List to store msgIndexes
    #
    #                 # Loop through each message to find the one with the matching guid
    #                 for message in messages:
    #                     if isinstance(message, dict):  # Check if message is a dictionary
    #                         guid = message.get("guid")
    #                         msg_index = message.get("msgIndex")  # Get msgIndex
    #
    #                         # Check if guid matches self.guildID
    #                         if guid == self.guildID:
    #                             msg_indexes.append(msg_index)  # Save msgIndex if guid matches
    #
    #                 print(f"Extracted msgIndexes: {msg_indexes}")  # Print the list of msgIndexes
    #             else:
